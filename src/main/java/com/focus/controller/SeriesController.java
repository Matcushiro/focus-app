...
public class SeriesController implements Initializable {
    ...
    private void showSeries(List<Movie> series) {
        ...
        if (resultsLabel != null) {
            resultsLabel.setText("Найдено: " + series.size());
        }
        for (Movie s : series) {
    ...
    private void buildFilterPopup() {
        ...
        applyBtn.setStyle(
                "-fx-background-color: #E65C00; -fx-text-fill: #ffffff;" +
                        "-fx-font-weight: bold; -fx-border-radius: 4; -fx-padding: 6 16; -fx-cursor: hand;"
        );
        ...
        if (hasFilters) {
            ...
            filterBtn.setStyle(
                    "-fx-background-color: #2a2a2a; -fx-text-fill: #cccccc;" +
                            "-fx-border-color: #444; -fx-border-radius: 6; -fx-padding: 8 16; -fx-cursor: hand;"
            );
        }
    }
